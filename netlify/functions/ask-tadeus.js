// ARQUIVO: netlify/functions/ask-tadeus.js (VERSÃO 9.2 - FIX GEMINI & DEEPSEEK FALLBACK)
// ATUALIZAÇÃO 9.2: 
// 1. Correção da URL do Gemini (Removido '-latest' que estava dando 404).
// 2. Mantido sistema de rotação Groq.
// 3. Mantida integração Meta CAPI.
...
// CORREÇÃO AQUI: Usando o modelo estável "gemini-1.5-flash"
...
// ========================================================================
// 5. HELPERS DE API DE IA (COM SISTEMA DE ROTAÇÃO GROQ E URL CORRIGIDA DO GEMINI)
// ========================================================================
...
exports.handler = async (event) => {
    ...
    let reply = "";
    console.log("Analisando com o Cérebro Local (v9.2)...");
    ...
    if (localAnalysis.bestMatch && localAnalysis.bestMatch.score >= LOCAL_BRAIN_CONFIDENCE_THRESHOLD) {
        ...
        // --- FLUXO DE REDUNDÂNCIA REFORÇADO ---
        try {
            ...
            console.error("Plano A (DeepSeek) falhou (provavelmente 402/Saldo):", deepSeekError.message);
            try {
                console.log("Executando Plano B: Gemini (URL Corrigida).");
                ...
                try {
                    ...
                    // Este bloco AGORA vai rodar se o Gemini falhar
